...
with app.app_context():
    ...
    for _d in _wiki_defaults:
        _a = WikiArticle.query.filter_by(slug=_d["slug"]).first()
        if _a:
            _a.title    = _d["title"]
            _a.category = _d["category"]
            _a.content  = _d["content"]
        else:
            db.session.add(WikiArticle(**_d))
    db.session.commit()
    print("Wiki sincronizzata con gli articoli di default.")
